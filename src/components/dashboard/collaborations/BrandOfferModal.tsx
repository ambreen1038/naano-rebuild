...
import { respondToOffer, counterOffer } from "@/app/brand/collaborations/actions";
...
export function BrandOfferModal({
  ...
  const waitingOnCreator = isOpen && latestPending?.offered_by === "brand";
  const canRespond = isOpen && latestPending?.offered_by === "creator";

  async function handleRespond(response: "accept" | "decline") {
    setSubmitting(true);
    setError(null);
    const result = await respondToOffer(booking.id, response);
    setSubmitting(false);
    if (!result.ok) {
        ...
        <div className="flex flex-col gap-5 p-6">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-zinc-900 dark:text-zinc-50">
                          {o.offered_by === "creator" ? booking.creatorName : "You"}{" "}
                          offered {o.amount} €
                        </span>
          ...
          {waitingOnCreator && (
            <p className="rounded-lg bg-blue-50 px-3 py-2 text-sm text-blue-700 dark:bg-blue-950 dark:text-blue-300">
              Waiting on {booking.creatorName} to respond to your offer.
            </p>
          )}
